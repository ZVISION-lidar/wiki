...
main                  线上发布分支                  是，push 后 1-3 分钟生效
zvision_develop       集成分支，所有功能分支合并到这里   否
feature/<名字>        个人功能分支，基于 develop 创建    否

核心规则：
1. 永远不要直接在 main 上 commit
2. 永远不要直接在 zvision_develop 上 commit
3. 改动流程：从 zvision_develop 拉 feature/<名字>，在 feature 上改，push，合并到
   zvision_develop，测试稳定后从 zvision_develop 发布到 main
...
1. 用任意编辑器（如 VS Code）打开 docs/ 下的文件进行修改。
   ...
   mkdocs serve
   浏览器打开 http://127.0.0.1:8000 查看修改效果。
...
注意：必须用 --no-ff（创建 merge commit），这样 main 的历史里能清楚看到每次
合入的功能。
...
什么时候发布：
攒了一批改动、测试通过后，由维护者定期（按需，比如每天 / 每周 / 发布节点）
从 zvision_develop 发布到 main。日常单次改动不需要走这一步。
...
注意：必须用 --no-ff（创建 merge commit），让 main 历史里能看到每次发布的合并
节点，避免 fast-forward 失败。


五、验证发布是否成功

push main 后 1-3 分钟，访问下面链接并刷新（Ctrl+F5 强刷新避开浏览器缓存）：
...
功能分支合并有冲突：在本地 rebase 到最新 zvision_develop，解决冲突后用
    git push --force-with-lease（比 -f 安全，若远端已被别人更新会拒绝推送）
...
最安全的做法：新开一个 feature 分支修复，或者在 zvision_develop 上直接
commit 修复。
不要用 git reset + git push -f zvision_develop，会破坏历史。

8.5 已经发布到 main 了，发现内容不对

最安全的做法：重新改一遍，走完整流程修复，等下次发布到 main。
不要用 git reset + git push -f main，会破坏历史。

8.6 git merge --no-ff zvision_develop 发布到 main 时报冲突

正常情况下不会发生，因为 main 应该是 zvision_develop 的子集。如果真发生，
多半是 main 上意外有了别的提交（有人违反了"不在 main 上直接 commit"的规则）：
...
1. 在 main 上直接 commit（必须通过 zvision_develop 合并过去）
2. 在 zvision_develop 上直接 commit（必须通过 feature 分支合并进去）
3. git push -f（强制推送，会覆盖远端历史）
4. git reset --hard 后立刻 git push -f
5. 多人同时在同一个 feature 分支上工作（每人用自己分支）
